...
- put inside of the Modelfile the following:
    - at the end i'll put the system prompt again in a more human readable way :D 
```
FROM deepseek-r1:7b
...
We now have a clear idea of the scenarios and test cases. But let’s not forget—we’re testing a chatbot here. We’re not feeding it a dataset and expecting it to return a neat result. It has to talk to the user, ask questions, hold a conversation. So... what’s next?
...
So let’s be smart about it. Let’s automate. Time to write a script that can do the talking for us—simulate the conversation, feed the inputs, and log the results. Dave doesn’t sleep, and now neither will our testing.
...
Starting the bot - open your terminal and run:
```
ollama run dave
```

Now, a very important note: if you don’t follow the flow of questions the model is expecting, it’s going to get confused fast—and you’ll likely end up with nonsense results or invalid recommendations. Here's what happened in my first attempt when I just provided the answers in order:
...
```
Yup. Dave went rogue. He ignored the actual values and defaulted to Product_5, even though based on the rules, Product_1 was the only correct answer. Why? Because I didn’t give the answers in the order he expected.
...
- algoritmic approach (a.k.a. keyword bingo)
    ...
    - **Disadvantages:**
        ...
        - It’s fragile. If Dave’s message contains words from multiple topics, this approach can totally freak out and return garbage (or worse—nothing).
- ask another AI (a.k.a. Dave's smarter cousin steps in)
    - To be honest, this should be your backup plan, not the default. I just wanted to see how well it would actually work. Spoiler: not bad, but not cheap.
    ...
    - **Disadvantages:**
        ...
        - It will not always anwer with yes or no, and you'll have to put the question again and again, yet more time lost... 
...
- cosine similarity
    ...
    - **Advantages:**
        - Much more flexible than keywords—no need for exact matches.
...
In my journey, I went with the algorithmic approach, backed up by a fallback to an AI check. Simple enough, right? Well… almost. Of course, in true lazy QA fashion, I just used the same model that Dave is based on to “double-check” his intent. Spoiler: it didn’t go great. Using the same brain to ask and answer a question isn’t exactly the best way to catch confusion. The results were… mixed at best.
...
To improve this, I decided to enhance the detection logic using cosine similarity. But first, that means converting sentences into vectors (a.k.a. embeddings). Since I’m working in JavaScript, @xenova/transformers is a great library that works locally—no cloud dependency, no API keys. You can install it like this: `npm install @xenova/transformers`. 

Once that’s in place, you’ll need a few reference sentences per topic—basically, examples of what Dave might say when he’s asking about a particular thing:
...
Now, for cosine similarity to work, you need a function that:
- Multiplies each number in one vector with the matching number in another.
- Adds them all up (this is the dot product).
- Calculates the length (magnitude) of each vector—think of it like measuring how long an arrow is.
- Divides the dot product by the product of the magnitudes—this normalizes the result.
...
And just like that, you’ve gone from guesswork and keywords to real semantic matching—without needing to ask another AI for help every time Dave mumbles.
...
- And don’t forget to re-check the happy paths. Just because a test used to pass doesn’t mean it still does. AI is fickle. Watch out for false positives that might sneak in as a side effect of prompt changes or model updates.
...
Once you’ve got a stable test setup and solid metrics coming in, you can start doing what I like to call mutation testing. Since you have access to Dave’s inner workings (lucky you), you can start tweaking things—like modifying the system prompt—and watch how the metrics react.
...
Tweak the model parameters—temperature, top_p, context length, etc.—and see what effect that has.

Or, even better, swap out the base model entirely. Try a different LLM with the same setup and see how it handles the same scenarios. Some will behave better. Some will fall apart. That’s the point.

The beauty of this is: you’re no longer guessing. You now have a stable testing framework that gives you actual, measurable insights into how each mutation—whether it’s prompt engineering or model swapping—impacts performance.

And yes, it’s kind of fun to break Dave on purpose. For science.

## Conclusion
This whole experiment started out of curiosity and a bit of frustration: how do you even begin to test an AI that changes its answers like it changes moods? It turns out that the answer is somewhere between rule-based sanity checks, vector math, and just brute-forcing your way through conversations until patterns start to emerge. Dave might not be perfect, but with a bit of structure, some clever scripting, and the right kind of logging, you can absolutely get useful signals out of the noise.

Along the way, I learned that AI testing isn’t about chasing precision—it’s about managing unpredictability. Your job isn’t to get the model to behave perfectly. It’s to detect when it doesn’t, and why. The system prompt matters. The data matters. The flow of conversation matters. And your metrics? They’re not just numbers—they’re your compass in this weird, probabilistic jungle.

In the end, the goal isn't to tame Dave—because let's be honest, he's always going to surprise you. The real goal is to build a framework that can evolve with him. One that gives you confidence that when you tweak a prompt, swap out a model, or add new scenarios, you’ll know what got better, what broke, and where to dig next. Because if you're going to work with AI, you might as well test it like you mean it.